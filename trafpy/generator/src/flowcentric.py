...
class FlowPacker:
    ...
    def pack_the_flows(self):
        ...
        shuffled_packed_flows = copy.deepcopy(self.packed_flows)
        shuffled_keys = shuffled_packed_flows.keys()
        random.shuffle(list(shuffled_keys))
        for key, shuffled_key in zip(self.packed_flows.keys(), shuffled_keys):
            shuffled_packed_flows[shuffled_key] = self.packed_flows[key]
        ...
        for idx in self.index_to_node.keys():
            ...
            if endpoint_target_load_rate > self.network_load_config['ep_link_capacity']:
                ...
                if not self.auto_node_dist_correction:
                    ...
                    ended = time.time()
                    print('Corrected node distribution in {} seconds.'.format(ended-started))
                    break
