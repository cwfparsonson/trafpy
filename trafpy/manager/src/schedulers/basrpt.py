...
class BASRPT(SchedulerToolbox):
    ...
    def __init__(self, Graph, RWA, slot_size, V, packet_size=300, scheduler_name='basrpt'):
                ...
                for idx in range(len(taken_paths)):
                    ...
                    if (channel == taken_channel and any(e in taken_path_edges for e in edges)) or (channel == taken_channel and any(e[::-1] in taken_path_edges for e in edges)):
                        ...
                        contending_flows['contending_flows'].append({'cont_f': chosen_flows[idx],
                                                                     'chosen_p': path,
                                                                     'chosen_c': channel})
                        q = self.find_flow_queue(chosen_flows[idx]) 
                        cost = self.calc_basrpt_cost(chosen_flows[idx], q)
                        contending_costs.append(cost)
