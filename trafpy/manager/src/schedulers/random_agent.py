...
class RandomAgent:
    ...
    def get_scheduler_action(self, observation):
        # update scheduler network and enw flow states
        ...
        chosen_flows = []
        eps = list(self.scheduler.SchedulerNetwork.graph['endpoints'])
        random.shuffle(eps) # randomly shuffle eps
        for ep in self.scheduler.SchedulerNetwork.graph['endpoints']:
            ...
            random.shuffle(queue_keys) # randomly shuffle queues
            for queue in queue_keys:
                if len(queues[queue]['queued_flows']) == 0:
                    ...
                    flows = list(queues[queue]['queued_flows']) # randomly shuffle flows
                    ...
                    for flow in flows:
                        ...
                        establish_flow, p, c, packets_this_slot = self.scheduler.look_for_available_lightpath(flow, chosen_flows, random_shuffle=True)
                        if establish_flow:
                            ...
                            # no flow or channel available. Doing random so no contention resolution -> do not schedule this flow
                            if self.scheduler.debug_mode:
